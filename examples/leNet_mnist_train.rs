...
#[model("models/train/cazzo.tflite", 3, "crossentropy", true, [30000.0,0.0], [4096.0,1024.0])]
...
fn main() {
    let all_labels_train: Vec<Vec<[SMatrix<[f32; 1], 28, 28>; 1]>> = (0..10)
        .into_iter()
        .map(|label| {
            read_dir(format!("datasets/mnist_dataset/train/label-{}", label))
                .unwrap()
                .filter(|el| el.is_ok())
                .map(|el| {
    ...
    let all_labels_test: Vec<Vec<[SMatrix<[f32; 1], 28, 28>; 1]>> = (0..10)
        .into_iter()
        .map(|label| {
            read_dir(format!("datasets/mnist_dataset/test/label-{}", label))
                .unwrap()
                .filter(|el| el.is_ok())
                .map(|el| {
                    let array: Array2<f32> = read_npy(el.unwrap().path()).unwrap();
