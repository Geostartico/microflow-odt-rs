...
use std::fs::read_dir;
// #[model("models/train/merda.tflite", 3, "crossentropy", true, [60000.0,0.0], [2048.0,1024.0])]
#[model("models/train/merda.tflite", 2, "crossentropy", true, [0.0], [700.0])]
struct LeNet {}

fn main() {
    let all_labels_train: Vec<Vec<[SMatrix<[f32; 3], 28, 28>; 1]>> = (0..10)
        .into_iter()
        .map(|label| {
            read_dir(format!("datasets/mnist_dataset_3d/train/label-{}", label))
                .unwrap()
                .filter(|el| el.is_ok())
                .map(|el| {
        ...
        .collect();
    let all_labels_test: Vec<Vec<[SMatrix<[f32; 3], 28, 28>; 1]>> = (0..10)
        .into_iter()
        .map(|label| {
            read_dir(format!("datasets/mnist_dataset_3d/test/label-{}", label))
                .unwrap()
                .filter(|el| el.is_ok())
                .map(|el| {
                    let array: Array3<f32> = read_npy(el.unwrap().path()).unwrap();
        ...
        .collect();
    let mut all_labels_train: Vec<([SMatrix<[f32; 3], 28, 28>; 1], usize)> = all_labels_train
        .iter()
        .enumerate()
        .flat_map(|(ind, el)| el.iter().map(move |sample| (*sample, ind)))
        .collect();
    let all_labels_test: Vec<([SMatrix<[f32; 3], 28, 28>; 1], usize)> = all_labels_test
        .iter()
        .enumerate()
        .flat_map(|(ind, el)| el.iter().map(move |sample| (*sample, ind)))
        .collect();
    ...
    for epoch in 0..epochs {
        ...
        println!(
            "changed params0 {}/{}",
            changed0,
            model.weights0.buffer.shape().0 * model.weights0.buffer.shape().1
        );
        // println!(
        //     "changed params1 {}/{}",
        //     changed1,
        //     model.weights1.buffer.shape().0 * model.weights1.buffer.shape().1
        // );
        println!(
            "validation accuracy : {}/{}",
            correct,
