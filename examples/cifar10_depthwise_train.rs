...
fn main() {
    ...
    let mut label_1: Vec<[SMatrix<[f32; 3], 32, 32>; 1]> =
        read_dir("datasets/cifar10_dataset/label1")
            .unwrap()
            .filter(|el| el.is_ok())
            .map(|el| {
                let array: Array3<f32> = read_npy(el.unwrap().path()).unwrap();
    ...
    println!(
        ...
        constants_first_last.1,
        constants_first_last.2,
        constants_first_last.3
    );
    let epochs = 25;
    let batch = 128;
    ...
    for epoch in 0..epochs {
        ...
        //     })
        // });
        let diff_last: Buffer2D<i32, 64, 2> =
        ...
        let changed_last = diff_last
            ...
            .fold(0, |acc, el| acc + el);
        let changed_first_last = diff_first_last
            .iter()
            .map(|el| if *el != 0 { 1 } else { 0 })
            .fold(0, |acc, el| acc + el);
        let saturated_conv = model
