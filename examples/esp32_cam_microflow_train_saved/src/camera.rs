use esp_idf_sys::camera as camera_esp;
...
pub struct FrameBuffer<'a> {
    fb: *mut camera_esp::camera_fb_t,
    _p: PhantomData<&'a camera_esp::camera_fb_t>,
}
...
impl<'a> FrameBuffer<'a> {
    ...
    pub fn format(&self) -> camera_esp::pixformat_t {
        unsafe { (*self.fb).format }
    }

    pub fn timestamp(&self) -> camera_esp::timeval {
        unsafe { (*self.fb).timestamp }
    }

    pub fn fb_return(&self) {
        unsafe { camera_esp::esp_camera_fb_return(self.fb) }
    }
}
...
pub struct CameraSensor<'a> {
    sensor: *mut camera_esp::sensor_t,
    _p: PhantomData<&'a camera_esp::sensor_t>,
}

impl<'a> CameraSensor<'a> {
    pub fn new(sensor: *mut camera_esp::sensor_t) -> Self {
        Self { sensor, _p: PhantomData }
    }
    ...
    pub fn set_pixformat(&self, format: camera_esp::pixformat_t) -> Result<(), EspError> {
        esp!(unsafe { (*self.sensor).set_pixformat.unwrap()(self.sensor, format) })
    }
    pub fn set_framesize(&self, framesize: camera_esp::framesize_t) -> Result<(), EspError> {
    ...
    pub fn set_gainceiling(&self, gainceiling: camera_esp::gainceiling_t) -> Result<(), EspError> {
...
pub struct Camera_wrapper<'a> {
    _p: PhantomData<&'a ()>,
}
impl<'a> Camera_wrapper<'a> {
    pub fn new(
        ...
        pin_siod: impl Peripheral<P = impl InputPin + OutputPin> + 'a,
        pin_sioc: impl Peripheral<P = impl InputPin + OutputPin> + 'a,
        pixel_format: camera_esp::pixformat_t,
        frame_size: camera_esp::framesize_t
        ...
        let config = camera_esp::camera_config_t {
            ...
            jpeg_quality: 12,
            fb_count: 2,
            grab_mode: camera_esp::camera_grab_mode_t_CAMERA_GRAB_WHEN_EMPTY,

            fb_location: camera_esp::camera_fb_location_t_CAMERA_FB_IN_PSRAM,

            __bindgen_anon_1: camera_esp::camera_config_t__bindgen_ty_1 {
                pin_sccb_sda: pin_siod.pin(),
            },
            __bindgen_anon_2: camera_esp::camera_config_t__bindgen_ty_2 {
                pin_sccb_scl: pin_sioc.pin(),
            },

            ..Default::default()
        };

        esp_idf_sys::esp!(unsafe { camera_esp::esp_camera_init(&config) })?;
        Ok(Self { _p: PhantomData })
    }
        pub fn get_framebuffer(&self) -> Option<FrameBuffer> {
        let fb = unsafe { camera_esp::esp_camera_fb_get() };
        if fb.is_null() {
    ...
    pub fn sensor(&self) -> CameraSensor<'a> {
        CameraSensor {
            sensor: unsafe { camera_esp::esp_camera_sensor_get() },
            _p: PhantomData,
        }
    }
}
