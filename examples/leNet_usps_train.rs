...
#[model("models/train/lenet_mnist2usps.tflite", 3, "crossentropy", true, [30000.0,0.0], [4096.0,1024.0])]
// #[model("models/train/lenet.tflite", 2, "crossentropy", true, [0.0], [1024.0])]
struct LeNet {}

fn main() {
    let all_labels_train: Vec<Vec<[SMatrix<[f32; 1], 28, 28>; 1]>> = (0..10)
        .into_iter()
        .map(|label| {
            read_dir(format!("datasets/usps_dataset/train/label-{}", label))
                .unwrap()
                .filter(|el| el.is_ok())
                .map(|el| {
                    let array: Array2<f32> = read_npy(el.unwrap().path()).unwrap();
                    // println!(
                    //     "{},{},{}",
                    //     array.shape()[0],
                    //     array.shape()[1],
                    //     array.shape()[2],
                    // );
                    [SMatrix::from_fn(|i, j| [*(array.get((i, j)).unwrap())])]
                })
                .collect()
        })
        .collect();
    let all_labels_test: Vec<Vec<[SMatrix<[f32; 1], 28, 28>; 1]>> = (0..10)
        .into_iter()
        .map(|label| {
            read_dir(format!("datasets/usps_dataset/test/label-{}", label))
                .unwrap()
                .filter(|el| el.is_ok())
                .map(|el| {
                    let array: Array2<f32> = read_npy(el.unwrap().path()).unwrap();
        ...
        .collect();
    let mut all_labels_train: Vec<([SMatrix<[f32; 1], 28, 28>; 1], usize)> = all_labels_train
        .iter()
        .enumerate()
        .flat_map(|(ind, el)| el.iter().map(move |sample| (*sample, ind)))
        .collect();
    let all_labels_test: Vec<([SMatrix<[f32; 1], 28, 28>; 1], usize)> = all_labels_test
        .iter()
        .enumerate()
        .flat_map(|(ind, el)| el.iter().map(move |sample| (*sample, ind)))
        .collect();
    ...
    for epoch in 0..epochs {
        ...
        println!("changed params1 {}", changed1);
        println!(
            "validation accuracy : {}/{}",
            correct,
            all_labels_test.len()
